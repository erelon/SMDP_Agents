"""Cumulative power-mean rate R-learning agent."""

from .power_rates import CumulativePowerMeanRate
...
class RiskTotalR(ContinuousRLearning):
    """R-learning with a cumulative weighted power mean of local rates.
    ...
    Duration weighting is the default because at ``p=1`` it reproduces the
    cumulative SMDP reward rate used by SMART.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rate = CumulativePowerMeanRate(self.p)
        self.reset()
        ...
        try:
            ...
            if time == 0:
                raise ZeroDivisionError(
                    "RiskTotalR requires nonzero elapsed time"
                ) from None
            raise

    @property
    def observation_count(self):
        return self.rate.mean.count

    @property
    def total_weight(self):
        return self.rate.mean.total_weight
